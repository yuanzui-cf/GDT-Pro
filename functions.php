...
function themeConfig($form) {
    ...
    $languageCode = new Typecho_Widget_Helper_Form_Element_Text('langCode', NULL, _t('zh-cn'), _t('默认语言代码'), NULL);
    ...
    $chooseSet = new Typecho_Widget_Helper_Form_Element_Checkbox('chooseSet',
        array('hideNotice' => _t('隐藏公告栏'),
            'hideLanguageSetBox' => _t('隐藏语言选择框')),
        array('hideNotice','hideLanguageSetBox'),
        _t('常规选项'));
    ...
    $form->addInput($chooseSet->multiMode());
    $form->addInput($otherSet->multiMode());
}
function themeFields($layout) {
	$postimage = new Typecho_Widget_Helper_Form_Element_Textarea('postimage', NULL, NULL, _t('文章头图'), _t('在这里填入文章头图地址'));
